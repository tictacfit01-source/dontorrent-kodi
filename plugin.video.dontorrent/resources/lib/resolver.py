...
class DomainResolver:
    ...
    def _probe(self, host):
        url = f"https://{host}/"
        try:
            r = util.proxy_get(url, timeout=12)
        ...
        if not self.validator(r.text):
            return None
        # When proxied, the final URL is in x-mw-relay-final header
    ...
    def resolve(self, force=False):
        ...
        # 0) Supabase: dominio centralizado (prioridad maxima)
        supabase_hosts = []
        try:
            from . import supabase_sync as sb
            sb_domain = sb.get_domain(self.name)
            if sb_domain:
                supabase_hosts.append(sb_domain)
            supabase_hosts.extend(sb.get_fallback_domains(self.name))
        except Exception:
            pass

        # Probar Supabase primero (si esta configurado)
        for h in supabase_hosts:
